...
import (
	...
	scte130 "github.com/dugwill/libScte130"
)
...
func main() {
	...
	if err != nil {
		fmt.Println("Unmarshal failled. Returned err:", err)
		return
	}
	client := psn.Client()
	fmt.Printf("TerminalAddress: %s\n", client.Address())
	fmt.Printf("TerminalType: %s\n", client.Type())

	fmt.Printf("AssetId: %s\n", psn.AssetID())
	fmt.Printf("Tracking: %s\n", psn.Tracking())
	fmt.Printf("Number of Events: %d\n", psn.NumEvents())

	for _, event := range psn.GetEvents() {
		dur, err := event.PlayDuration()
		if err != nil {
			fmt.Printf("Error getting duration: %v\n", err)
		}
		fmt.Printf("Event Type: %s\n", event.EventType())
		fmt.Printf("PlaybackDuration: %s\n", dur)
	}
